...
The problem with using `IOutgoingSocket.SendMore()` rather than `IOutgoingSocket.Send()`, if you intend to send more than 1 frame, is that you MUST ensure you call `SendMore`, where as if you use the `SendMessage()` method, you do not have to worry about that, that is all taken care of for you.
