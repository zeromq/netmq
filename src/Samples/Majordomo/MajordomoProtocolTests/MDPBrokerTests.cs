...
namespace MajordomoTests
{
    ...
    public class MDPBrokerTests
    {
        ...
        public async void Run_ReceiveREPLYMessageFromWorker_ShouldLogCorrectReply ()
        {
            ...
            using (var echoWorker = new MDPWorker (endPoint, "echo", idW01))
            {
                ...
                if (debugLog.Count > 0)
                {
                    ...
                    Assert.That (debugLog.Contains ("[MDP BROKER DEBUG] Dispatching -> NetMQMessage[C1,,He lo World!] to echo"));
                    Assert.That (debugLog.Contains ("[MDP BROKER DEBUG] REPLY from W1 received and send to C1 -> NetMQMessage[MDPC01,echo,He lo World!]"));
                }
            }
        }
        ...
        private static void EchoClient (IMDPClient client, string serviceName)
        {
            ...
            Assert.That (reply.First.ConvertToString (), Is.EqualTo ("He lo World!"));
        }
        ...
        private static void MultipleRequestClient (string serviceName, string endpoint, IMDPClient client = null)
        {
            ...
            Assert.That (reply.All (r => r.First.ConvertToString () == "He lo World!"));
        }
