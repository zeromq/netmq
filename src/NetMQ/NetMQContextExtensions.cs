...
namespace NetMQ
{
    ...
    public static class NetMQContextExtensions
    {
        ...
        /// <param name="progressReporter">Set to true to report progress to Console</param>
        /// <returns>the response message, or null if not successful</returns>
        public static string RequestResponseStringWithRetry([NotNull] this NetMQContext context, [NotNull] string address,
            [NotNull] string requestString, int numTries, TimeSpan requestTimeout, IProgress<string> progressReporter = null)
        {
            var requestSocket = GetNewRequestSocket(context, address);
            try
            {
                while (numTries-- > 0)
                {
                    if (progressReporter != null)
                    {
                        progressReporter.Report(string.Format("Sending string: {0}", requestString));
                    }
                    ...
                    if (requestSocket.TryReceiveFrameString(requestTimeout, out frameString))
                    {
                        if (progressReporter != null)
                        {
                            progressReporter.Report(string.Format("Server replied OK with: {0}", frameString));
                        }
                        return frameString;
                    }
                    if (progressReporter != null)
