...
namespace NetMQ
{
	...
	{
		public INetMQMonitor CreateMonitor(INetMQSocket socket, string endpoint)
		{
			return new NetMQMonitor(socket, endpoint);
		}

		public INetMQMonitor CreateMonitor(
			INetMQContext context,
			INetMQSocket monitoredSocket,
			string endpoint,
			SocketEvent eventsToMonitor)
		{
			return new NetMQMonitor(context, monitoredSocket, endpoint, eventsToMonitor);
		}

		/// <summary>
		/// Use the  class when you want to monitor socket
		/// </summary>
		private class NetMQMonitor : INetMQMonitor
		{
			private bool m_isOwner;

			private IPoller m_attachedPoller = null;
			...
			public NetMQMonitor(INetMQSocket socket, string endpoint)
			{
				Endpoint = endpoint;
				Timeout = TimeSpan.FromSeconds(0.5);
			...
			/// </summary>
			internal INetMQSocket MonitoringSocket { get; private set; }
				...
				if (monitorEvent != null)
				{
					switch (monitorEvent.Event)
					{
						...
						case SocketEvent.Disconnected:
							InvokeEvent(
								Disconnected,
								new NetMQMonitorSocketEventArgs(this, monitorEvent.Addr, (AsyncSocket)monitorEvent.Arg));
							break;
						default:
			...
			public void AttachToPoller(IPoller poller)
			{
				InternalStart();
			...
			public void Stop()
			{
				if (m_attachedPoller != null)
				{
					throw new InvalidOperationException(
						"Monitor attached to a poller, please detach from poller and don't use the stop method");
				}
