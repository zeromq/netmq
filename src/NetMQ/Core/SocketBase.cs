...
using TcpListener = NetMQ.Transports.Tcp.TcpListener;

namespace NetMQ.Core
{
    abstract class SocketBase : Own, IPollEvents, Pipe.IPipeEvents
    {
        internal const int PollIn = 1;
        internal const int PollOut = 2;
        ...
        public PollEvents GetEvents(PollEvents pollEvents)
        {
            ProcessCommands(0, false);

            PollEvents result = PollEvents.None;

            if (pollEvents.HasFlag(PollEvents.PollIn) && HasIn())
            {
                result = PollEvents.PollIn;
            }

            if (pollEvents.HasFlag(PollEvents.PollOut) && HasOut())
            {
        ...
        public PollEvents Poll(PollEvents pollEvents, int timeout)
        {            
            Stopwatch stopwatch = Stopwatch.StartNew();

            PollEvents result = GetEvents(pollEvents);

            while (result == PollEvents.None && (timeout == -1 || stopwatch.ElapsedMilliseconds < timeout))
            {
                ...
                if (FD.Poll(currentTimeoutMicroSeconds, SelectMode.SelectRead))
                {
                    result = GetEvents(pollEvents);
                }
            }
