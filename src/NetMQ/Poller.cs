...
namespace NetMQ
{
	public partial class Factory : INetMQFactory
	{
		...
		public IPoller CreatePoller(params ISocketPollable[] sockets)
		{
			return new Poller(sockets);
		}

		public IPoller CreatePoller(params NetMQTimer[] timers)
		{
			return new Poller(timers);
		}

		private class Poller : IPoller
		{
			private class PollerSelectItem : SelectItem
			{
				private INetMQSocket m_socket;

				public PollerSelectItem(INetMQSocket socket, PollEvents events)
					: base(socket.SocketHandle, events)
				{
					m_socket = socket;
				}

				public INetMQSocket NetMQSocket
				{
					get
					{
						return m_socket;
			...
			private readonly IList<INetMQSocket> m_sockets = new List<INetMQSocket>();
			...
			private INetMQSocket[] m_pollact;
			...
			private void PollWhile(Func<bool> condition)
			{
				...
				if (Thread.CurrentThread.Name == null) Thread.CurrentThread.Name = "NetMQPollerThread";
